        ...
        <div style="height:calc(100% - 42px);overflow:auto;padding:0 5px;">
            <div v-show="type==1">
                ...
                <p v-for="(item,index) in filter" :key="index" class="zhibiaohover" style="background:#7172AD;color:#fff;margin:3px 0;">
                    ...
                    &nbsp;<span style="color:#fff;display:inline-block;width:calc(100% - 25px);vertical-align: sub;overflow:hidden" :title="filterLabel[index]">{{filterLabel[index]}}</span>
                    <Icon class="colse colseHover" type="md-close" @click="removeFilter(index)" style="font-size:18px;float:right"/>
                </p>
                <Divider />
                <h3>
                    <span>筛选条件</span>
                ...
                <p v-for="(item,index) in data" :key="index" class="zhibiaohover" @click="selectItem(item)">
                    <template v-if="item.title.indexOf(tableName)>-1">
                        ...
                        &nbsp;<span style="    vertical-align: middle;width:calc(100% - 30px);display: inline-block;
...
export default {
    ...
    methods:{
        init(){
            ...
            this.filterType=''
            this.filterType2='day'
            this.filterValue=''
            this.timeValue=[]
            this.filter=[]
            this.filterLabel=[]
            this.filterDateValue=3
        },
        selectItem(item){
            this.type = 2
            this.checkedItem = item
            this.filterValue = ''
            if(this.checkedItem.dataType=='DATE') this.filterType = 12
        ...
        addFilter(){
            ...
            if(this.checkedItem.dataType.toLowerCase()=='date'){
            ...
            this.filter.push({
                ...
                column:this.checkedItem.title,
                value:filterValue
            })
            this.filterLabel.push(this.checkedItem.title+' '+label+' '+label2)
            this.type=1
