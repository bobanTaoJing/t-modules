...
<template>
  <div>
   <!-- <DataTable :columns="tableOptions.columns"
               ...
               @on-row-click="rowClick"></DataTable>-->
    <Table ref="table" :columns="columnsList"  :data="thisTableData" border disabled-hover></Table>
  </div>
</template>
...
<script>
  ...
  const queryData=()=>{
    this.$http.apiPost('/generator/table/columns', {}).then((res) => {
      this.loading = false;
      if (!res.success) {
        this.$http.handleError(res);
      } else {
        //this.total = res.data.total;
        this.thisTableData = res.data.rows;
      }
    });
  };
  ...
  export default {
    ...
    methods: {
      ...
      insertOrUpdateData (p, loading) {
        ...
        let list = this.$refs['table'].thisTableData;
        // 本地结果集
        if (list !== null) {
          let isUpdate = false;
