...
<div class="table-panel" :class="{'edit-table':!canCopy}">
    <div>
        <div style="position:relative" :style="tableData.length>10?'height:'+height+'px':''">
            <Table v-if="reflash" :data="showTableData" :columns="hColumns" ref="table" @on-drag-drop="onDragDrop" :draggable="draggable" :size="tableSize" :class="{'table-loading': loading}" :max-height='height'></Table>
             <Spin size="large" fix v-else></Spin>
        </div>
        <Page v-if="tableData.length>10" :total="tableData.length" :current="page" :page-size="pageSize" @on-change="pageChange" size="small" show-total style="height:30px;line-height:30px;text-align:center"/>
        ...
        hColumns() {
            ...
            if(this.showSerial)
                arr.push({
                    ...
                    render: (h, params) => {
                        return h('div', [
                        h('span',params.index+1+(this.page-1)*this.pageSize)
                        ]);
                    }
                })
            if(this.autoName)
                    ...
                    return h('div', [
                        h('Poptip', {
                            ...
                            h('Input', {
                                ...
                                on: {
                                    'on-change': (event) => {
                                        console.log((this.page-1)*10 + params.index)
                                        this.tableData[(this.page-1)*10 + params.index][item.key] = event.target.value
                                    }
                                }
                            }),
                        ])
                    ...
                    return h('div', [
                        h('Select', {
                            ...
                            on: {
                                'on-change': (value) => {
                                    // console.log(value)
                                    this.tableData[(this.page-1)*10 + params.index][item.key] = value
                                }
                            }
                        }, options),
                    ]);
