...
export default {
    ...
    onClick(e) {
        ...
        if (item.hasState('selected')) {
            ...
            eventBus.$emit('nodeselectchange2', { target: item, select: false });
        } else {
            if (self.shouldUpdate.call(self, e)) {
                graph.setItemState(item, 'selected', true);
            }
            eventBus.$emit('nodeselectchange2', { target: item, select: true });
        }
    ...
    onCanvasClick() {
        ...
        Util.each(selected, node => {
            graph.setItemState(node, 'selected', false);
            eventBus.$emit('nodeselectchange2', { target: node, select: false });
        });

        const selectedEdges = graph.findAllByState('edge', 'selected');
        Util.each(selectedEdges, edge => {
            graph.setItemState(edge, 'selected', false);
            eventBus.$emit('nodeselectchange2', { target: edge, select: false });
        })
